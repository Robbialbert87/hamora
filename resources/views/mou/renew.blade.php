...
@section('title', 'Perpanjang MOU - HAMORA')
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <div class="mb-3">
                    <h4 class="card-title mb-0">Form Perpanjangan MOU</h4>
                    <p class="text-muted mb-0">Perpanjang MOU dengan masa berlaku baru. Data perpanjangan sebelumnya akan tersimpan di riwayat.</p>
                </div>

                {{-- Info MOU Lama --}}
                <div class="alert alert-light border d-flex align-items-center gap-3 mb-4" role="alert">
                    <div class="flex-shrink-0">
                        <div class="rounded-circle d-inline-flex align-items-center justify-content-center" style="width: 48px; height: 48px; background: linear-gradient(135deg, #556ee5, #7b8ff7);">
                            <i class="ti ti-file-text text-white" style="font-size: 20px;"></i>
                        </div>
                    </div>
                    <div>
                        <div class="fw-semibold" style="font-size: 14px;">{{ $mou->nomor }} — {{ $mou->judul }}</div>
                        <div class="text-muted" style="font-size: 12.5px;">
                            {{ $mou->pihak }} &middot;
                            {{ $mou->mulai_perjanjian->format('d/m/Y') }} s/d {{ $mou->akhir_perjanjian->format('d/m/Y') }} &middot;
                            <span class="badge bg-{{ $mou->status === 'aktif' ? 'success' : ($mou->status === 'kadaluarsa' ? 'danger' : 'secondary') }}">{{ ucfirst($mou->status) }}</span>
                    @if($mou->versi > 1)
                        <span class="badge bg-info">Perpanjangan #{{ $mou->versi - 1 }}</span>
                    @endif
                        </div>
                    </div>
                </div>

                <form action="{{ route('mou.renew.store') }}" method="POST" enctype="multipart/form-data" novalidate>
                    @csrf
                    <input type="hidden" name="parent_mou_id" value="{{ $mou->id }}">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form-label">Nomor MOU Baru <span class="text-danger">*</span></label>
                                <input type="text" name="nomor"
                                    class="form-control @error('nomor') is-invalid @enderror"
                                    value="{{ old('nomor') }}" required
                                    placeholder="Nomor MOU perpanjangan baru">
                                @error('nomor')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form-label">File PDF Baru <span class="text-danger">*</span></label>
                    ...
                    <div class="mt-3 text-end">
                        <a href="{{ route('mou.show', $mou->id) }}" class="btn btn-outline-secondary btn-sm"><i class="ti ti-arrow-left me-1"></i> Kembali</a>
                        <button type="submit" class="btn btn-primary btn-sm"><i class="ti ti-device-floppy me-1"></i> Simpan Perpanjangan</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        mulaiInput.addEventListener('change', function() {
            if (masaBerlakuInput.value) updateDariMasaBerlaku();
            else if (akhirInput.value) updateDariAkhir();
        });

        masaBerlakuInput.addEventListener('change', function() {
            if (this.value) updateDariMasaBerlaku();
            else if (akhirInput.value) updateDariAkhir();
            else { akhirInput.value = ''; keteranganInput.value = ''; }
        });

        akhirInput.addEventListener('change', function() {
            if (this.value) updateDariAkhir();
            else if (masaBerlakuInput.value) updateDariMasaBerlaku();
            else keteranganInput.value = '';
        });
    });
</script>
@endsection
